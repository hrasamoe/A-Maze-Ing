...
class MazeConfig:
        ...
        try:
            ...
            for err in e.errors():
                ...
                if error_type == "missing":
                    ...
                    occured_error.append(
                        f"[!] Invalid value: {field_name} must be a number"
                    )
                    ...
                    if field_name == "HEIGHT":
                        occured_error.append(
                            f"[!] Out of bounds: '{field_name}' must bebetween 8 and 16"
                        )
            ...
            error_str = "\n".join(occured_error)
            raise ValueError(f"Validation failed in '{filepath}' : \n{error_str}")
        self.logic_chek()
    ...
    def logic_chek(
        ...
        if not (0 <= x1 < self.property.width and 0 <= y1 < self.property.height):
            raise ValueError(f"Entry {self.property.entry} is out of bounds")
        if not (0 <= x2 < self.property.width and 0 <= y2 < self.property.height):
